...
export default function ActionPlanBuilder({ cards, initialIdentity = '', initialConcern = '', initialIds = '' }: Props) {
  const [identity, setIdentity] = useState(initialIdentity);
  const [concern, setConcern] = useState(initialConcern);
  const [generated, setGenerated] = useState(!!initialIds);
  const [copied, setCopied] = useState(false);

  /* if ids param is provided, use those cards directly */
  const idsSet = useMemo(() => {
    if (!initialIds) return null;
    return new Set(initialIds.split(',').map((s) => s.trim()).filter(Boolean));
  }, [initialIds]);

  const filtered = useMemo(() => {
    if (idsSet) {
      return cards.filter((c) => idsSet.has(c.card_id));
    }
  ...
  return (
    <div className="space-y-6">
      {/* questionnaire – hidden when ids are provided */}
      {!idsSet && (
